 ...
 **/
@Component
@RocketMQMessageListener(consumerGroup = "LittleRedBook_group_first_reply_comment_id" + MQConstants.TOPIC_COUNT_NOTE_COMMENT, // Group 组
        ...
        if (CollUtil.isNotEmpty(missingCommentIds)) {
            ...
            needUpdateCommentDOS.forEach(needUpdateCommentDO -> {
                ...
                if (Objects.nonNull(earliestCommentDO)) {
                    ...
                    // 异步同步到 Redis
                    threadPoolTaskExecutor.execute(() -> sync2Redis(Lists.newArrayList(earliestCommentId)));
                }
            });
        }
    }
