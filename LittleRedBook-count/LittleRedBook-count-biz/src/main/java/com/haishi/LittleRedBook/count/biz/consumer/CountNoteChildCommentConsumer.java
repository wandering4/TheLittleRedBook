...
import com.haishi.LittleRedBook.count.biz.enums.CommentLevelEnum;
 ...
 **/
@Component
@RocketMQMessageListener(consumerGroup = "LittleRedBook_group_child_comment_total_" + MQConstants.TOPIC_COUNT_NOTE_COMMENT, // Group 组
...
public class CountNoteChildCommentConsumer implements RocketMQListener<String> {
    ...
    private BufferTrigger<String> bufferTrigger = BufferTrigger.<String>batchBlocking().bufferSize(50000) // 缓存队列的最大容量
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        Map<Long, List<CountPublishCommentMqDTO>> groupMap = countPublishCommentMqDTOList.stream().filter(commentMqDTO -> Objects.equals(CommentLevelEnum.TWO.getCode(), commentMqDTO.getLevel())).collect(Collectors.groupingBy(CountPublishCommentMqDTO::getParentId)); // 按 parent_id 分组
        ...
        for (Map.Entry<Long, List<CountPublishCommentMqDTO>> entry : groupMap.entrySet()) {
            ...
            // 判断 Hash 是否存在
            boolean hasKey = Boolean.TRUE.equals(redisTemplate.hasKey(commentCountHashKey));
        ...
        // 异步发送计数 MQ, 更新评论热度值
        org.springframework.messaging.Message<String> message = MessageBuilder.withPayload(JsonUtils.toJsonString(commentIds)).build();
